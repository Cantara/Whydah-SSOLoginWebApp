...
public class CRMHelper {
	...
	public boolean userHasManyAddresses(String userTokenXml) {
		...
		if (userToken.getPersonRef().length() > 2) {
			try {
				...
				String crmCustomerJson = new CommandGetCRMCustomer(crmServiceUri, serviceClient.getMyAppTokenID(), userToken.getTokenid(), userToken.getPersonRef()).execute();
	...
	public void createCrmCustomer_AaddToModel(Model model, String userTokenXml, Customer customer) {
		...
		try {
			...
			String returnedPersonRef = new CommandCreateCRMCustomer(crmServiceUri, serviceClient.getMyAppTokenID(), userToken.getTokenid(), userToken.getPersonRef(), customerJson).execute();

			log.info("Created CRMrecord for user - personRef={}, returnedPersonRef:{] - userTokenId:{} -  crmcustomer: {} - userTokenXml:{}", userToken.getPersonRef(), returnedPersonRef, userToken.getTokenid(), customerJson, userTokenXml);
	...
	public String updateCrmData(String userTokenXml, String updateJson) {
		...
		if (userToken.getPersonRef().length() > 2) {
			try {
				SSLTool.disableCertificateValidation();
				String location = new CommandUpdateCRMCustomer(crmServiceUri, serviceClient.getMyAppTokenID(), userToken.getTokenid(), userToken.getPersonRef(), updateJson).execute();
	...
	public boolean SendPhoneVerificationPin(String userTokenXml, String phoneNo) {
		...
		if (userToken.getPersonRef().length() > 2) {
			try {
				SSLTool.disableCertificateValidation();
				Boolean pinSent = new CommandSendPhoneVerificationPin(crmServiceUri, serviceClient.getMyAppTokenXml(), userToken.getTokenid(), userToken.getPersonRef(), phoneNo).execute();
	...
	public boolean SendPhoneVerificationPin(String userTokenXml, String phoneNo, String pin) {
		...
		if (userToken.getPersonRef().length() > 2) {
			try {
				SSLTool.disableCertificateValidation();
				Boolean pinSent =  new CommandVerifyPhoneByPin(crmServiceUri, serviceClient.getMyAppTokenXml(), userToken.getTokenid(), userToken.getPersonRef(), phoneNo, pin).execute();
	...
	public boolean sendEmailVerificiationToken(String userTokenXml, String email) {
		...
		if (userToken.getPersonRef().length() > 2) {
			try {
				SSLTool.disableCertificateValidation();
				Boolean tokenSent = new CommandSendEmailVerificationToken(crmServiceUri, serviceClient.getMyAppTokenXml(), userToken.getTokenid(), userToken.getPersonRef(), email, emailVerificationLink).execute();
	...
	public boolean verifyEmailByToken(String userTokenXml, String email, String token) {
		...
		if (userToken.getPersonRef().length() > 2) {
			try {
				SSLTool.disableCertificateValidation();
				Boolean tokenVerified = new CommandVerifyEmailByToken(crmServiceUri, serviceClient.getMyAppTokenXml(), userToken.getTokenid(), userToken.getPersonRef(), email, token).execute();
				log.info("sendEmailVerificiationToken - used personRef={}, appTokenID:{}, userTokenXml:{},email:{}", userToken.getPersonRef(), serviceClient.getMyAppTokenID(), userTokenXml, email);
